...
# DFC data
dfc <- subset(colData, Region=="DFC")$SampleID
df <- assay(vsd)[,dfc] 
...
datTraits <- ctf[dfc,c(2:9,11,16,19,20)]
...
# Construct gene network and identify modules using a power of 9

cor <- WGCNA::cor # temporarily reassign the cor function to avoid mixing it across different packages

net.dfc = blockwiseModules(datExpr, power = 9, maxBlockSize=15000,
...
moduleLabels = net.dfc$colors
me <- c("ME1","ME2","ME3","ME4","ME5","ME6","ME7","ME8","ME9","ME10","ME11","ME12","ME13")
moduleNames = labels2colors(net.dfc$colors,colorSeq=me)
MEs0 = net.dfc$MEs
geneTree = net.dfc$dendrograms[[1]]
save(datExpr,datTraits,MEs0, moduleLabels, moduleNames, geneTree, net.dfc,
     file = "ml_dfc-networkConstruction-auto.RData")


# Relate modules to external variables -----------------------------------------

# Load network data saved in the previous part.
lnames = load(file = "ml_dfc-networkConstruction-auto.RData")
...
write.table(geneInfo,"~/rnaseq_brain_data_sets/Maryland/results/wgcna_geneInfo_dfc.txt",sep="\t",row.names=F)
...
glist <- lapply(c(1,8,9), function(x){
  modSize <- subset(geneInfo, moduleID == levels(df$variable)[x]) %>% nrow
  df.sub <- subset(df, variable == levels(df$variable)[x])
  ggplot(data=df.sub, aes(x=Age, y=value, color=variable)) + 
    geom_smooth(se=T, method="loess", level=0.5,span= 0.76) +
...
annotate_figure(ggarrange(plotlist=glist,ncol=3,nrow=1),
                left=text_grob("Module eigengene",size=16,rot=90),
                bottom=text_grob("Age (Y)",size=15,rot=0),
                top=text_grob("Dynamic modules, DFC\n",size=16,rot=0))
...
egolist <- lapply(c(1,8,9), function(x){
...
write.table(egoFull,"~/rnaseq_brain_data_sets/Maryland/results/wgcna_egoFull_dfc.txt",sep="\t",row.names=F)

egoFullRed <- distinct(egoFull, moduleID,parentTerm, .keep_all= TRUE) %>% data.frame
egoTop <- Reduce(rbind, 
                 by(egoFullRed, egoFullRed["moduleID"], head, n = 6)
)
write.table(egoTop[,c(6,7,10,13,17,21)],"~/rnaseq_brain_data_sets/Maryland/results/wgcna_egoTop_dfc.txt",sep="\t",row.names=F) # shorten GO descriptions manually

# Plot GO results
egoTopShort <- read.delim("~/rnaseq_brain_data_sets/Maryland/results/wgcna_egoTop_short_dfc.txt",sep="\t",header=T)
...
egoTopShort <- rbind( #need correct order for plotting
  ...
  subset(egoTopShort,moduleID=="M8"),
  subset(egoTopShort,moduleID=="M9")
)

egoTopShort$moduleID <- factor(egoTopShort$moduleID, levels= c("M9","M8","M1"))

order <- egoTopShort$short_name
egoTopShort$short_name <- factor(egoTopShort$short_name, levels=order)
col <- get_palette("aaas", 13)[c(9,8,1)]
