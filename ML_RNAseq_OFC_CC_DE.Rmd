...
cor_subset <- sample(cpm_all, 1000)
...
corr_res_expr_vis <- 
  ...
  filter(geneX %in% time_trend_genes$ensembl_gene_id & geneY %in% time_trend_genes$ensembl_gene_id) %>% 
   group_by(geneX) %>%   dplyr::slice(which.max(abs(corr_res))) %>% ungroup() %>% #make sure only each gene is only part of pair once
   group_by(geneY) %>%   dplyr::slice(which.max(abs(corr_res))) %>% ungroup() %>% #make sure only each gene is only part of pair once
  arrange(corr_res) 
...
pos_corr_res_expr <- corr_res_expr_cat %>%  filter(cat== "pos_expr_pos_res") %>% arrange(p_expr) %>%  head(n=100) %>% tail(n=4)

#visualise examples
  #visualise gene expression of negatively correlated pairs
 visualise_expr <-  cpm_all_long %>% 
     mutate(pair = case_when(
    ensembl_gene_id  %in% pos_corr_res_expr[1, 1:2] ~ "1",
    ensembl_gene_id  %in% pos_corr_res_expr[2, 1:2] ~ "2",
    ensembl_gene_id  %in% pos_corr_res_expr[3, 1:2] ~ "3",
    ensembl_gene_id  %in% pos_corr_res_expr[4, 1:2] ~ "4",
    TRUE ~ NA)) %>% 
    drop_na
...
poly_coeff_pvalue_df %>% 
  ...
  filter(coeff != "(Intercept)", p<0.05) %>%  
  ggplot() + 
  geom_histogram(mapping = aes(x=p, colour= coeff))
...
time_trend_genes <-  time_trend %>% filter(p< 0.05) %>% select(ensembl_gene_id) %>% distinct()
  
time_trend_genes_linear <- time_trend %>% filter(p< 0.05 & coeff=="poly(AGE, 3)1") %>% select(ensembl_gene_id) %>% distinct()
...
#plot GO analysis: Region
gene <- rownames(res_region[res$adj.P.Val < 0.05,])

#for this analysis, we need entrez ID
gene.entrez <- bitr(gene, 
                    fromType = 'ENSEMBL', 
                    toType= 'ENTREZID', 
                    OrgDb         = org.Hs.eg.db, 
                    drop = TRUE)

all.genes.entrez <-  bitr(rownames(res_region), 
                    fromType = 'ENSEMBL', 
                    toType= 'ENTREZID', 
                    OrgDb         = org.Hs.eg.db, 
                    drop = TRUE)
...
"DE-analysis" alternative: test whether the coefficient on the linear age variable  (the one to the power of 1) in the fitted line is significantly different from 0.
 X code to get significant genes, nominally and FDR, linear/ all coefficients 

Plot some of the extreme examples of not-significant/significant in correlation
Filter on age-function coefficients to find age trends
