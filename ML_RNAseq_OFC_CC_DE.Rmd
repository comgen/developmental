...
coeff_min <- 2 #the minimum linear coefficient included in the analysis
sig_level <- 0.01
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")
...
pairs <- expand.grid(pos_linear$ensembl_gene_id, neg_linear$ensembl_gene_id) #Create a data frame from all combinations of the supplied vectors
...
Mexpr[lower.tri(Mexpr)] <- NA #removes the reverse correlation
...
#if relevant, add both genes' linear age-coefficient
corr_res_expr <- 
  corr_res_expr %>% 
  left_join(select(age_trend_genes_linear_adj, ensembl_gene_id, lin_coeff), by= c("geneX" = "ensembl_gene_id")) %>%
  dplyr::rename("geneX_lin_coeff" = lin_coeff)
  
corr_res_expr <- 
  corr_res_expr %>% 
  left_join(select(age_trend_genes_linear_adj, ensembl_gene_id, lin_coeff), by= c("geneY" = "ensembl_gene_id")) %>%
  dplyr::rename("geneY_lin_coeff" = lin_coeff)
...
xy <- intersect(
  linear_slope_intrcpt$Intercept[1], linear_slope_intrcpt$Intercept[2], 
  linear_slope_intrcpt$Slope[1], linear_slope_intrcpt$Slope[2]
  ) # Returns the xy coordinates of the intersection # Returns the xy coordinates of the intersection
...
gene <- age_trend_genes_linear_adj$ensembl_gene_id
 analysis <- "age_trend_genes_OFC"
 analysis <- "age_trend_genes_CC"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")
