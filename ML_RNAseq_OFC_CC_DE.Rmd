...
#IA has suggested using blue (#E64B35FF) and red (#8491B4FF) whenever we do a direct comprison between CC and OFC
```
...
#res <- read_tsv(paste0(res_dir, "/de_limma.txt"))
...
if(identical(analysis,  "CC_DE_below12")){
  DE_0.01_CC <- res %>% filter(adj.P.Val <= 0.01)
}  
...
kableExtra::kable(table_sign_genes, 
                  "latex", caption = paste("Number of significant genes", "FDR<", sig_level)) %>% 
  kableExtra::save_kable(paste0(res_dir, "/sign.genes.png"))
...
gene <- rownames(DE_0.01_OFC[DE_0.01_OFC$adj.P.Val < 0.01,])
analysis <- "OFC_DE_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")

# below vs above 12 years-old, CC
gene <- rownames(DE_0.01_CC[DE_0.01_CC$adj.P.Val < 0.01,])
analysis <- "CC_DE_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")
