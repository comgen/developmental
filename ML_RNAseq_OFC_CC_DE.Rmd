...
#metod using ranked gene lists. But can't input universe (all relevant genes)?
# gene_sorted <- res[res$adj.P.Val < 0.01, "adj.P.Val"]
# names(gene_sorted) <- rownames(res)[res$adj.P.Val < 0.01]
# 
# gene_sorted <- sort(-log10(gene_sorted), decreasing = TRUE)
# 
# gse <- gseGO(
...
time_trend_genes_linear_adj_OFC <- read_tsv("~/proj_Maryland_WGCNA/analysis/time_trend_genes/time_trend_linear.txt")

time_trend_genes_linear_GWAS_OFC <-   
  BD_GWAS_100kb %>% 
  filter(ensgene %in% time_trend_genes_linear_adj_OFC$ensembl_gene_id ) %>% 
  rowwise() %>% 
...
#visualise expression when nearest gene has time trend: OFC
 expr_time_GWAS_closest_OFC <-
   cpm_all_long_OFC %>% filter(gene_id %in% time_trend_genes_linear_GWAS_OFC$symbol) %>% left_join(time_trend_genes_linear_GWAS_OFC, by= c("gene_id" = "symbol")) %>% filter(gene_id== Nearest_gene) %>%  ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
    geom_point()+ 
    facet_wrap(~gene_id, scales = "free_y") +
    geom_smooth(method = lm, formula = y ~ poly(x, 3), se = FALSE) 
 
 
#visualise expression when l2g-gene has time trend: OFC
expr_time_GWAS_l2g_OFC <- 
  cpm_all_long_OFC %>% 
  filter(gene_id %in% BD_GWAS_l2g$symbol) %>% 
  left_join(time_trend_genes_linear_GWAS_OFC, by= c("gene_id" = "symbol")) %>% filter(gene_id== Nearest_gene) %>%  
  ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
    geom_point() + 
    facet_wrap(~gene_id, scales = "free_y") +
    geom_smooth(method = lm, formula = y ~ poly(x, 3), se = FALSE) 
...
#genes in age-switch gene pair within 100kb from GWAS loci OFC 
 ...
 ggsave(plot = expr_time_GWAS_closest_CC, filename = "~/proj_Maryland_WGCNA/analysis/time_trend_genes_CC/expr_time_GWAS_closest_CC.pdf")
  ggsave(plot = expr_time_GWAS_closest_OFC, filename = "~/proj_Maryland_WGCNA/analysis/time_trend_genes/expr_time_GWAS_closest_OFC.pdf")

ggsave(plot = expr_time_GWAS_l2g_CC, 
          filename = "~/proj_Maryland_WGCNA/analysis/time_trend_genes_CC/expr_time_GWAS_l2g_CC.pdf")
 
ggsave(plot = expr_time_GWAS_l2g_OFC, 
         filename = "~/proj_Maryland_WGCNA/analysis/time_trend_genes/expr_time_GWAS_l2g_OFC.pdf")
...
expr_time_GWAS_myelin_OFC <- 
  cpm_all_long_OFC %>% 
  filter(gene_id %in% time_trend_genes_linear_GWAS_OFC$symbol) %>% 
  filter(ensembl_gene_id %in% myelin_genes$`ENSEMBL ID`) %>% 
  ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
    geom_point() + 
    facet_wrap(~gene_id, scales = "free_y") +
