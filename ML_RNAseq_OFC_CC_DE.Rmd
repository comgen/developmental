...
col <- c("#3B4992FF", "#EE0000FF", "#D95F02", "#1B9E77", "#440154FF", "#31688EFF", "#35B779FF", "#FDE725FF")

#IA has suggested using blue (#E64B35FF) and red (#8491B4FF) whenever we do a direct comparison between CC and OFC
```
...
Add on number of reads per sample
```{r qc reads}

reads_OFC <- read_tsv("/Users/asbjorh/PhD/RNAseq_temporal/data/OFC/Hughes-RNA5-2020-07-07_reads_OFC.csv")
reads_OFC$`% Dups` <- str_remove_all(string = reads_OFC$`% Dups`, pattern = "%")
reads_OFC$`% GC` <- str_remove_all(string = reads_OFC$`% GC`, pattern = "%")
reads_OFC$`Sample Name` <- str_remove_all(string = reads_OFC$`Sample Name`, pattern = "-BA11_clean")

reads_OFC <- 
  reads_OFC %>% separate(`Sample Name`, sep = "_", into = c("Sample", "read")) %>% filter(read== "1P") %>% mutate(sampleID = paste0(Sample, "-", "OFC")) %>% right_join(info_OFC, by= "sampleID") #%>% dplyr::summarise(median_reads= median(`M Seqs`))



reads_CC <- read_tsv("/Users/asbjorh/PhD/RNAseq_temporal/data/CC/Hughes-RNA6-2021-12-08/Hughes-RNA6-2021-12-08_reads_OFC.txt")
reads_CC <- reads_CC %>% select(-`% Failed`)
reads_CC$`% Dups` <- str_remove_all(string = reads_CC$`% Dups`, pattern = " %")
reads_CC$`% GC` <- str_remove_all(string = reads_CC$`% GC`, pattern = " %")

reads_CC <- reads_CC %>% separate(`Sample Name`, into = c("SampleNumber", "RNAid"), sep = "-") %>% separate(`RNAid`, into = c("sampleID", "lane", "read", "end"), sep = "_") %>% select(-c(SampleNumber, lane, end)) %>% mutate(sampleID = paste0(sampleID, "-", "CC"))

reads_CC %>% right_join(info_CC, by= "sampleID") %>%  
  summarise(reads= median(`M Seqs`))

reads_CC %>% right_join(info_CC, by= "sampleID") %>%  
  summarise(mean_reads= mean(`M Seqs`), sd_reads = sd(`M Seqs`))


#combine the 2 datasets
reads_total <-
  bind_rows(reads_CC, reads_OFC, .id = "region")

reads_total %>%   summarise(mean_reads= mean(`M Seqs`), sd_reads = sd(`M Seqs`))

```

Convert ensembl gene names to symbol
```{r Process data CC}
clinMeta <- info_CC
count <- count_CC

# exclude outlier CC_1259 sample (PCA) and CC_5282 (low counts)
exclude <- c("1259-CC", "5282-CC")
...
#library(edgeR)
keep <- filterByExpr(data, design=colData$below12 #Group-aware filtering makes sure there is expression in enough samples within at least one group. 
)
data.filt <- data[keep,]
...
```{r some more metrics}
#violin plot of counts per gene for each sample
rownames_to_column( data.filt, var = "ensembl_gene_id") %>% 
  pivot_longer(cols = -1, names_to = "Sample", values_to = "counts") %>% 
...
rownames_to_column( data.filt, var = "ensembl_gene_id") %>% 
  pivot_longer(cols = -1, names_to = "Sample", values_to = "counts") %>% 
  group_by(Sample) %>%
  dplyr::summarise(mean_counts = mean(counts), sd_counts = sd(counts)) %>%
  arrange(mean_counts) %>%  head()

#Find that sample 5282-CC has low counts, should perhaps be removed
```
...
```{r DE analyses childhood vs adolescence CC}
...
#res <- read_tsv(paste0(res_dir, "/de_limma_CC_below12.txt"))

res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")
...
write.table(res, paste0(res_dir, "/de_limma_below12.txt"), sep="\t", row.names=F)
...
DE_OFC <- res %>% filter(adj.P.Val <= sig_level)
}  

if(identical(analysis,  "CC_DE_below12")){
  DE_CC <- res %>% filter(adj.P.Val <= sig_level)
}  

DE_CC_high_children <- DE_CC %>% filter(t > 0 & adj.P.Val < sig_level)
DE_CC_low_children <- DE_CC %>% filter(t < 0 & adj.P.Val < sig_level)
...
cpm_all_long_CC <- cpm_all_long

#save cpm values
#write_tsv(select(cpm_all_long_CC, -CDEATHOFF), file = paste0("~/proj_Maryland_WGCNA/analysis/", "cpm_all_long_CC.txt") )


GOI_CC <- c("HHATL", "TSPAN8", "TKT", "SLC6A11", "NCAN", "CERS6")

volcano_CC <- 
  EnhancedVolcano(res,
                lab = res$external_gene_name, ylab = bquote(- ~Log[10]~ 'p (adj)'), 
                title= "childhood vs adolescence" , subtitle = "differential expression: CC",
                x = 'logFC',
                y = 'adj.P.Val', pCutoff = sig_level,
                FCcutoff = 0.5,  
              selectLab = GOI_CC,
                boxedLabels = TRUE,
                drawConnectors = TRUE,
                  legendLabSize = 7, axisLabSize = 7, titleLabSize	= 12 , subtitleLabSize = 7, 
                captionLabSize = 7, labSize = 2, col = c("#440154FF", "#31688EFF" ,"#35B779FF",  "#FDE725FF"),

    legendIconSize = 1, legendPosition= "none")

ggsave(volcano_CC, file = paste0(res_dir, "/volcano.pdf" ), width = 90, units = "mm")  
```
...
## Keep only protein-coding and lncRNA genes
genemap2 <- genemap[rownames(data.filt),]
genemap2 <- subset(genemap2, gene_biotype %in% c("protein_coding"))
data.filt <- data.filt[rownames(genemap2),]
colData.filt <- subset(colData, rownames(colData) %in% SampleID )
...
```{r DE analyses childhood vs adolescence OFC}
analysis <- "OFC_DE_below12"
#sig_level <- 0.01
sig_level   <- 0.05
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")
res <- read_tsv(paste0(res_dir, "de_limma_OFC_below12.txt"))
...
table_sign_genes <- summary(decideTests(fit, p.value=sig_level))
write.table(table_sign_genes, file = paste0(res_dir, "/sign.genes.txt"), sep = "\t", row.names = T, col.names = T)
...
cpm_all <- cpm(y, log=TRUE, prior.count=2) #convert counts to normalised cpm
...
cpm_all_long_OFC <- cpm_all_long

#save cpm values
#write_tsv(cpm_all_long_OFC, file = paste0("~/proj_Maryland_WGCNA/analysis/", "cpm_all_long_OFC.txt") )

GOI_OFC <- c("KNG1", "KCNS1", "COL1A1", "ADGRG2", "C4A", "KCNB1")

volcano_OFC <- 
  EnhancedVolcano(res,
                lab = res$external_gene_name, ylab = bquote(- ~Log[10]~ 'p (adj)'), 
                title= "childhood vs adolescence" , subtitle = "differential expression: OFC",
                x = 'logFC',
                y = 'adj.P.Val', pCutoff = 5e-2,
                FCcutoff = 0.5,  
              selectLab = GOI_OFC,
                boxedLabels = TRUE,
                drawConnectors = TRUE,
...
ggsave(volcano_OFC, file = paste0(res_dir, "/volcano.pdf" ), width = 90, units = "mm")  

```


Compare the 2 brain tissues
...
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")
#res_region <- read_tsv( paste0(res_dir, "/de_limma.txt")) #after it has been saved, in stead of rerunning analysis

#create a new directory with sig_level if it doesn't exist
...
#remove from counts_all any samples that are not in exclude
counts_all <- counts_all[, !colnames(counts_all) %in% exclude]

colData_all <-  bind_rows("OFC"=info_OFC,
                          "CC"= select(info_CC, -CDEATHOFF),
                          .id = "region")
 
colData_all <- colData_all %>% filter(!sampleID %in% exclude) 
#only keep samples in colData_all that are in counts_all
colData_all <- colData_all %>% filter(sampleID %in% colnames(counts_all)) #only keep samples in colData_all that are in counts_all

colData_all <- colData_all %>% column_to_rownames(var="sampleID") #to make a dataframe with rownames
colData_all <- colData_all[colnames(counts_all),] #order the sample names the same as the expression data


sample_all <-  colnames(counts_all)[colnames(counts_all) %in% rownames(colData_all)]
...
counts_all.filt <- counts_all.filt[rownames(counts_all.filt) %in% rownames(genemap2), !colnames(counts_all.filt) %in% exclude]
...
kableExtra::kable(table_sign_genes, 
                  "latex"#, 
             #     caption = paste("Number of significant genes,", "FDR", sig_level)
                  ) %>% 
  kableExtra::save_kable(paste0(res_dir, "/sign.genes.png"))
...
high_OFC_FDR <- res_region %>%  filter(t > 0 & adj.P.Val < sig_level)
low_OFC_FDR <- res_region %>%  filter(t < 0 & adj.P.Val < sig_level)

#save high and low OFC FDR genes
write.table(high_OFC_FDR, paste0(res_dir, "/high_OFC.txt"), sep="\t", row.names=F)
write.table(low_OFC_FDR, paste0(res_dir, "/low_OFC.txt"), sep="\t", row.names=F)
...
sig <- res %>% filter(adj.P.Val < sig_level) %>% slice_max(P.Value) %>% select(P.Value)
sig <- sig$P.Value
...
#Presentation: plot CC genes
plot_expr <- 
  df_long %>% 
  left_join(select(genemap2, ensembl_gene_id, external_gene_name) , by=c("gene_id" = "ensembl_gene_id")) %>%  #add in symbol name
  # filter(gene_id %in% rownames(head(res,10))) %>% 
  filter(external_gene_name %in% GOI_CC) %>% 
    mutate(external_gene_name = factor(external_gene_name, levels= GOI_CC )) %>% 
...
#ggsave(plot_expr, file = paste0(res_dir, "/plot_expr_top10sign.pdf" ) , width = 120, units = "mm")  
  
#Presentation: plot CC genes by age
plot_expr_age <- cpm_all_long_CC %>% 
   filter(gene_id %in% GOI_CC) %>%  mutate(gene_id = factor(gene_id, levels= GOI_CC )) %>% 
     ggplot(aes(x=AGE, y= log_cpm, colour=Age_group)) +
     geom_point()+ 
   facet_wrap(~gene_id, scales = "free_y") +
     geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE, colour= "grey") +
  scale_colour_manual(values=col[c(3,4)])

#Presentation: plot OFC genes by age
plot_expr_age <- 
  cpm_all_long_OFC %>% 
   filter(gene_id %in% GOI_OFC) %>%  mutate(gene_id = factor(gene_id, levels= GOI_OFC )) %>% 
     ggplot(aes(x=AGE, y= log_cpm, colour=Age_group)) +
     geom_point()+ 
   facet_wrap(~gene_id, scales = "free_y") +
     geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE, colour= "grey") +
...
#ggsave(plot_expr_age, file = paste0(res_dir, "/plot_expr_age.pdf" ) , width = 120, units = "mm")  
...
GOI_region <- c("ANKRD10", "STARD9", "LMAN2L", "CACNA1B", "GRIN2A", "ANK3")
...
df_top8  <- df[rownames(head(res_region,8)), ] #only keep the top 8 significant genes

df_top8 <- df_top8 %>% t() %>% data.frame()     #have genes as columns
colnames(df_top8) <- head(res_region,8)$external_gene_name #use gene symbols in stead of ensembl codes

df_top8 <- df_top8 %>% rownames_to_column(var = "SampleID" )
...
plot_expr_highOFC <- 
  df_long %>% 
    filter(external_gene_name %in% high_OFC_FDR_001$external_gene_name[1:8] ) %>% 
  ggplot(aes(x=region, y= log_cpm)) + 
    geom_boxplot( color ="black",size=0.7, outlier.shape=NA, fill="white") +
    geom_point(aes( colour= region, fill = region), position= "jitter") +
  facet_wrap(~external_gene_name, scales = "free_y", nrow = 2) +
      scale_colour_manual(values=col[c(1,2)]) +
    scale_fill_manual(values=col[c(1,2)]) +
  labs(title= paste0("Normalised expression, 8 significant genes higher in OFC")) +
  theme_bw()

plot_expr_highCC <- 
  df_long %>% 
    filter(external_gene_name %in% low_OFC_FDR_001$external_gene_name[1:8] ) %>% 
  ggplot(aes(x=region, y= log_cpm)) + 
    geom_boxplot( color ="black",size=0.7, outlier.shape=NA, fill="white") +
    geom_point(aes( colour= region, fill = region), position= "jitter") +
  facet_wrap(~external_gene_name, scales = "free_y", nrow = 2) +
      scale_colour_manual(values=col[c(1,2)]) +
    scale_fill_manual(values=col[c(1,2)]) +
  labs(title= paste0("Normalised expression, 8 significant genes higher in CC")) +
  theme_bw()


#plot some examples of DE genes combined
plot_expr_examples <- 
  df_long %>% 
  filter(external_gene_name %in% GOI_region) %>% 
   mutate(external_gene_name = factor(external_gene_name, levels=GOI_region)) %>% #mind the order
  ggplot(aes(x=region, y= log_cpm)) + 
    geom_boxplot( color ="black",size=0.7, outlier.shape=NA, fill="white") +
    geom_point(aes( colour= region, fill = region), position= "jitter") +
  facet_wrap(~external_gene_name, scales = "free_y", nrow = 2) +
      scale_colour_manual(values=col[c(1,2)]) +
    scale_fill_manual(values=col[c(1,2)]) +
  labs(title= paste0("Normalised expression, example genes DE in CC vs OFC")) +
  theme_bw()

#ggsave(plot_expr_highOFC, file = paste0(res_dir, "/plot_expr_top8sign_highOFC.pdf" ), width = 180, units = "mm")  
#ggsave(plot_expr_highCC, file = paste0(res_dir, "/plot_expr_top8sign_highCC.pdf" ), width = 180, units = "mm")  
#ggsave(plot_expr_examples, file = paste0(res_dir, "/plot_expr_examples.pdf" ), width = 120, units = "mm")

volcano_region <- 
  EnhancedVolcano(res_region,
                lab = res_region$external_gene_name, ylab = bquote(- ~Log[10]~ 'p (adj)'), title= "CC vs OFC", subtitle = "differential expression",
                x = 'logFC',
                y = 'adj.P.Val', pCutoff = 5e-2,
                FCcutoff = 0.5,  
               selectLab = GOI_region,
                boxedLabels = TRUE,
                drawConnectors = TRUE,
                  legendLabSize = 7, axisLabSize = 7, titleLabSize	= 12 , subtitleLabSize = 7, 
                captionLabSize = 7, labSize = 2, col = c("#440154FF", "#31688EFF" ,"#35B779FF",  "#FDE725FF"),
...
#ggsave(volcano_region, file = paste0(res_dir, "/volcano_region.pdf" ), width = 90, units = "mm")  
```


Are there more variance in lower ages, i.e. need for stricter balance in adolescence?
...
DE_genes_0.05 <- 
  ...
  filter(adj.P.Val < 0.05) %>% select(external_gene_name)
...
```{r find genes with stat. diff. in variance}
library(rstatix)
##OFC
#res_dir <- "~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/"
#cpm_all_long <- cpm_all_long_OFC

## CC
res_dir <- "~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/"
 cpm_all_long <- cpm_all_long_CC

sd_diff_dir <-  cpm_all_long %>% 
  group_by(ensembl_gene_id, Age_group) %>%
  mutate(sd= sd(log_cpm)) %>% 
  dplyr::select(ensembl_gene_id, gene_id, Age_group, sd) %>% 
  distinct() %>% 
  pivot_wider(names_from = Age_group, values_from = sd, names_prefix = "sd_") %>% 
  dplyr::filter(abs(`sd_>12` - `sd_<12`) > 0.5) %>% 
  dplyr::mutate(sd_diff= `sd_>12` - `sd_<12`) %>% 
  mutate(direction= ifelse(sd_diff > 0,
                           "largest sd in oldest",
                           "largest sd in youngest"
  )) %>% 
  mutate(direction = factor(direction, levels= c("largest sd in youngest", "largest sd in oldest"
                           ))) %>% 
  ungroup()

count_sd_diff_dir <- sd_diff_dir %>% count(direction)
#write_tsv(count_sd_diff_dir, paste0(res_dir, "count_sd_diff_dir.txt"))
...
#   direction                  n
#   <chr>                  <int>
# 1 largest_sd_in_oldest      16
# 2 largest_sd_in_youngest  1845

#OFC
#   direction                  n
#   <chr>                  <int>
# 1 largest_sd_in_oldest      44
# 2 largest_sd_in_youngest  1037
...
variance_diff <- 
  ...
  levene_test(log_cpm ~ Age_group) 

variance_diff_adj <- 
  variance_diff %>%  rstatix::adjust_pvalue( p = "p", output.col = "p_adj", method = "fdr")

variance_diff_adj <- variance_diff_adj %>% 
  left_join(dplyr::select(genemap, ensembl_gene_id, external_gene_name), by= "ensembl_gene_id")

#write_tsv(variance_diff_adj, paste0(res_dir, "variance_diff_adj.txt"))

sd_diff_examples <-
  variance_diff_adj %>% 
  left_join(select(sd_diff_dir, ensembl_gene_id, sd_diff, direction), by= "ensembl_gene_id") %>% 
  filter(external_gene_name != "") %>% 
  group_by(direction) %>% 
  filter(external_gene_name %in% cpm_all_long$gene_id[cpm_all_long$log_cpm >2]) %>% #filter examples to genes with at least 1 log2cpm
  slice_min(p, n= 3 ) %>% tidyr::drop_na() %>% ungroup()
  
#Presentation: plot examples
plot_expr_sd_diff <- 
 cpm_all_long %>% 
 # filter(gene_id %in% sd_diff_examples$external_gene_name) %>% 
   filter(gene_id %in% sd_diff_examples$external_gene_name[sd_diff_examples$sd_diff < 0]) %>% #only the most common form: highest variance in youngest
    mutate(gene_id = factor(gene_id, levels= sd_diff_examples$external_gene_name )) %>% 
     ggplot(aes(x=AGE, y= log_cpm, colour=Age_group)) +
     geom_point()+ 
   facet_wrap(~gene_id, scales = "free_y") +
     geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE, colour= "grey") +
...
#ggsave(hist_sd_diff_dir, height = 60, width = 90, units = "mm", file = paste0(res_dir, "hist_sd_diff_dir.pdf"))
#ggsave(plot_expr_sd_diff, width = 120, units = "mm", file = paste0(res_dir, "plot_expr_sd_diff.pdf"))


```


### Gene pairs that can be biological switches
```{r pairs of switches}
#Kang et al
#IGF2 in fig 2c
#DCX in fig 5d
...
Gene subset: neuronal function (KEGG)
...
Correlation in expression: find gene pairs with opposite deviation from trendline: more likely to be molecular switch
...
#visualise gene expression of negatively correlated pairs
  ...
  geom_smooth(se = FALSE) +
        scale_colour_manual(values=col[5:8]) 
...
Gene subset: genes with changes in expression from early childhood to adulthood
...
#work on subset of genes:
#cor_subset_genes <- colnames(cor_subset)
...
for(gene in cor_subset_genes){
  ...
  counter <- counter + 1;
                  if(counter %in% seq(0, length(cor_subset_genes), round(length(cor_subset_genes))/10))
                      cat(paste(round(counter/length(cor_subset_genes))*100), "% has been processed")
}
...
coeff_hist <- poly_coeff_pvalue_df %>% 
  ...
  filter(coeff != "(Intercept)", p<sig_level) %>%  
  ggplot() + 
...
age_trend_genes_linear <- age_trend %>% 
  filter(p< sig_level & coeff=="poly(AGE, 2)1") %>% 
  select(ensembl_gene_id) %>% distinct()
...
#write_tsv(poly_coeff_pvalue_df, file = paste0(res_dir, "/poly_coeff_pvalue_df.txt" ))

#write.csv(res_poly_all_df, file = paste0(res_dir, "/res_poly_all_df.txt" ))
```

### Visualise expression of genes with possible age trend
```{r show gene expression trends of example genes}
library(gridExtra)
#human single cell from cerebellum, 1-yo and a handful in their 40ies
high_adult_cellXgene <- readxl::read_xlsx("~/proj_Maryland_WGCNA/analysis/cellXgene/higher_adult_topgenes.xlsx", col_names = "gene_id")

Li_2020_mouse <- c("IGFBPL1", "OSTN", "KLK6", "DEFB1")

find_paper <- c("GABRB1", "GABRG3", "ATF6", "CRTAC1")

##check expression OFC
res_dir <- "~/proj_Maryland_WGCNA/analysis/OFC_DE_below12/"
cpm_all_long <- cpm_all_long_OFC

##check expression CC
res_dir <- "~/proj_Maryland_WGCNA/analysis/CC_DE_below12/"
cpm_all_long <- cpm_all_long_CC

subset <- list()
subset[["Li_2020_mouse"]] <- Li_2020_mouse
subset[["find_paper"]] <- find_paper
subset[["high_adult_cellXgene"]] <- high_adult_cellXgene$gene_id

plot_list_expression_examples <- list() 

for(i in c(1,2,3)){

plot_list_expression_examples[[i]] <- 
  cpm_all_long %>% 
     filter(gene_id %in% subset[[i]]) %>% 
     ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
   geom_point()+
   geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE) +
  #      scale_colour_manual(values=col[5:8]) +
  facet_wrap(vars(gene_id), scales = "free_y") +
  labs(title = names(subset)[i], subtitle = subset[i]) +
  theme(legend.position = "none") 
  
}

plot_expression_examples <- arrangeGrob(grobs = plot_list_expression_examples, 
                                        layout_matrix = rbind(c(1,2),
                                                              c(3,3),
                                                              c(3,3))
             )

#ggsave(plot_expression_examples,  file= paste0(res_dir, "plot_expression_examples.pdf"))

#only mouse from Li paper
plot_list_expression_examples_Li <- 
  cpm_all_long %>% 
     filter(gene_id %in% Li_2020_mouse) %>% 
     ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
   geom_point()+
   geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE) +
        scale_colour_manual(values=col[5:8]) +
  facet_wrap(vars(gene_id), scales = "free_y") +
  labs(title = "Li_2020_mouse (4 genes)" )+
  theme(legend.position = "none") 
  
#ggsave(plot_list_expression_examples_Li,  file= paste0(res_dir, "plot_list_expression_examples_Li.pdf"))



#overlap with myelin genes for CC
myelin_genes <- readxl::read_xlsx("~/PhD/RNAseq_temporal/genes_of_interest/stokowy_steen_myelin_curated_for_r.xlsx", 
 ...
 age_trend_genes_linear_adj_CC <-
   read_tsv("~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/age_trend_linear.txt")

age_trend_myelin <- 
  age_trend_genes_linear_adj_CC %>% 
  filter(p_linear_adj < 0.05) %>% 
  filter(ensembl_gene_id %in% myelin_genes$`ENSEMBL ID`) %>% 
  left_join(dplyr::select(genemap, ensembl_gene_id, external_gene_name), by= "ensembl_gene_id")

  
plot_myelin_trend_CC <- 
  cpm_all_long %>% 
     filter(ensembl_gene_id %in% age_trend_myelin$ensembl_gene_id) %>% 
     ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
   geom_point()+
   geom_smooth(se = FALSE) +
...
#if(identical(res_dir,  "~/proj_Maryland_WGCNA/analysis/CC_DE_below12/")){
#ggsave(plot_myelin_trend_CC, file= paste0(res_dir, "plot_myelin_trend_CC.pdf"))
#       }  


#are there any with statistical significant age trend genes among the previously hypothesised?
age_trend_genes_linear_adj_OFC %>%  
  left_join(dplyr::select(genemap, ensembl_gene_id, external_gene_name), by= "ensembl_gene_id") %>% f
ilter(p_linear_adj < sig_level) %>% 
  filter(external_gene_name %in% 
           c(Li_2020_mouse, find_paper, high_adult_cellXgene$gene_id)) 
...
#No real age-trend for these genes in our data, except KLK6 for CC
...
cpm_all_wide <- 
  cpm_all_long %>% 
  dplyr::select(SampleID, ensembl_gene_id, log_cpm) %>% 
  pivot_wider(names_from = ensembl_gene_id, values_from = log_cpm) %>% 
...
#or we can just take the first one
pairs_GO_equal <- 
  pairs_GO_equal %>% 
  group_by(Var1, Var2) %>% 
  dplyr::slice(1) %>% 
...
```{r find interesting time-trend genes}
library(gridExtra)
#use GO-annotation to find interesting genes
##check expression OFC
#res_dir <- "~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/"
#cpm_all_long <- cpm_all_long_OFC
#age_trend_genes_linear_adj <-
 read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/age_trend_linear.txt"  )
...
# 
 age_trend_genes_linear_adj <- 
   read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/age_trend_linear.txt"  )

age_trend_linear_GO <- age_trend_genes_linear_adj %>% 
  filter(p_linear_adj < sig_level) %>% 
  left_join(GOall_BP, by= c("ensembl_gene_id" = "ENSEMBL"))

#find the GO-terms with most age-trend genes
#display 4 strongest genes in the 4 most populated GO-terms
top_terms <-
  age_trend_linear_GO %>%  
  tidyr::drop_na() %>% 
  count(Term) %>%  
...
top_terms <- base::as.vector(top_terms$Term)

top_term_genes <- age_trend_linear_GO %>%  
  tidyr::drop_na() %>% 
  filter(Term %in% top_terms ) %>% 
  arrange(p_linear_adj) %>% 
  group_by(Term) %>% 
  dplyr::slice(1:4)
  
plot_list_top_terms <- list() 

for(i in c(1,2,3,4)){

plot_list_top_terms[[i]] <- 
  cpm_all_long %>% 
     filter(gene_id %in% top_term_genes[top_term_genes$Term == top_terms[i],]$SYMBOL ) %>%
      ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
   geom_point()+
   geom_smooth(se = FALSE) +
  #      scale_colour_manual(values=col) +
  facet_wrap(vars(gene_id), scales = "free_y") +
  labs(title = paste0("GO-term: ", top_terms[i])) +
  theme(legend.position = "none")
  
}

plot_4_top_terms <- arrangeGrob(grobs = plot_list_top_terms 
                           )
ggsave(plot_4_top_terms,  file= paste0(res_dir, "plot_linear_4_top_terms.pdf"))
...
#import saved residuals
#res_poly_all_df <- read_csv("~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/res_poly_all_df.txt")

#import saved residuals, CC
res_poly_all_df <- read_csv("~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/res_poly_all_df.txt")

#make sure it is a dataframe with gene names as rownames
res_poly_all_df <- res_poly_all_df %>% column_to_rownames(var = "...1")
...
for(i in seq(1:length(rownames(pairs)))){
  ...
  geneX = as.vector(pairs[i,1])
  geneY = as.vector(pairs[i,2])
  ...
  cor_res_p = cor.test(as.numeric(res_poly_all_df[geneX,]), as.numeric(res_poly_all_df[geneY,]), conf.level = 0.95)
...
corr_res_expr <- corr_res_expr %>% left_join(select(pairs_GO_equal, -Var2) , by= c("geneX" = "Var1"))
```


Method when total list of genes is small enough to do all against all correlation (and remove irrelevant gene pairs afterwards)
...
#Gene pairs with opposite linear age trend selected in chunck "genes of interest correlation residuals and expression levels". Correlations calculated in previous chunk "compute corr res expr for all pairs of genes"
...
#only keep genes where residuals are significantly different from zero (p.adj)
corr_res_expr <- 
  corr_res_expr %>% 
...
corr_res_expr_cat$corr_expr <- as.numeric(corr_res_expr_cat$corr_expr)
corr_res_expr_cat$corr_res <- as.numeric(corr_res_expr_cat$corr_res)
corr_res_expr_cat$p_expr <- as.numeric(corr_res_expr_cat$p_expr)
corr_res_expr_cat$p_res <- as.numeric(corr_res_expr_cat$p_res)
...
for(c in c("neg_expr_neg_res",  "neg_expr_notsign_res",  "neg_expr_pos_res", "pos_expr_pos_res",  "notsign_expr_neg_res")){
...
#plot_pos_cor <- plotlist$neg_expr_pos_res
...
#ggsave(plot_pos_cor, file = paste0(res_dir, "plot_expr_pos_cor.pdf")) 
...
#save the gene pairs with equal GO-terms 
write_tsv(pairs_GO_equal, file = paste0(res_dir, "pairs_GO_equal.txt"))
...
res_dir <- "~/proj_Maryland_WGCNA/analysis/switches/GO_term/"

pairs <- pairs_GO_equal %>% 
  select(Var1, Var2) %>% 
  distinct()

#particularly interesting GO-terms
#(top count in gene pairs, related to brain function and/or development?)
...
#save the table with gene pairs and GO-terms
write_tsv(corr_res_expr_vis, file = paste0(res_dir, "corr_res_expr_vis.txt"))
...
#keep only the highest hsapiens_paralog_perc_id 
corr_res_expr_paralog <-
  corr_res_expr_paralog %>% 
  group_by(geneX, geneY) %>% 
  slice_max(hsapiens_paralog_perc_id) %>% 
  ungroup()
...
#plot
  ...
  geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE) +
   ...
   #save paralog pairs
   write_tsv(corr_res_expr_paralog, file = paste0(res_dir, "corr_res_expr_paralog.txt"))
   
```


###Start enrichment analysis of gene sets 
...
gene <- DE_OFC$ensembl_gene_id[DE_OFC$adj.P.Val < sig_level]
analysis <- "OFC_DE_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")

#highest in children, OFC
sig_level <- 0.05
analysis <- "OFC_DE_high_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")
gene <- DE_OFC$ensembl_gene_id[DE_OFC$adj.P.Val < sig_level & DE_OFC$logFC > 0]

#lower in children, OFC
sig_level <- 0.05
analysis <- "OFC_DE_low_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")
gene <- DE_OFC$ensembl_gene_id[DE_OFC$adj.P.Val < sig_level & DE_OFC$logFC < 0]

# below vs above 12 years-old, CC
gene <- DE_CC$ensembl_gene_id[DE_CC$adj.P.Val < sig_level]
analysis <- "CC_DE_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")

#highest in children, CC
sig_level <- 0.05
analysis <- "CC_DE_high_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")
gene <- DE_CC$ensembl_gene_id[DE_CC$adj.P.Val < sig_level & DE_CC$logFC > 0]

#lower in children, CC
sig_level <- 0.05
analysis <- "CC_DE_low_below12"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/", "FDR_",sig_level, "/")
gene <- DE_CC$ensembl_gene_id[DE_CC$adj.P.Val < sig_level & DE_CC$logFC < 0]
...
#OFC higher than CC
analysis <- "higherOFC"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/OFC_CC_DE/", analysis, "/", "FDR_",sig_level, "/")
res <- res_region
gene <- high_OFC_FDR$ensembl_gene_id

#CC higher than OFC
analysis <- "higherCC"
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/OFC_CC_DE/", analysis, "/", "FDR_",sig_level, "/")
res <- res_region
gene <- low_OFC_FDR$ensembl_gene_id

#Genes with significant age-coefficient OFC
age_trend_genes_linear_adj <- read_tsv(  "~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/age_trend_linear.txt" )

analysis <- paste0("age_trend_genes_linear_OFC",
                   "_", "FDR_", sig_level, "_upwards")

res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")

#use genes with significant age-trend, upwards
gene <- age_trend_genes_linear_adj %>% filter(p_linear_adj < sig_level, lin_coeff > 0) %>% select(ensembl_gene_id)
gene <- gene$ensembl_gene_id

#use genes with significant age-trend, downwards
analysis <- paste0("age_trend_genes_linear_OFC",
                   "_", "FDR_", sig_level, "_downwards")
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")
...
#Genes with significant age-coefficient CC
age_trend_genes_linear_adj <- read_tsv(  "~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/age_trend_linear.txt" )
analysis <- paste0("age_trend_genes_linear_CC",
                   "_", "FDR_", sig_level, "_upwards")
res_dir <- paste0("~/proj_Maryland_WGCNA/analysis/", analysis, "/")
#use genes with significant age-trend, upwards
gene <- age_trend_genes_linear_adj %>% filter(p_linear_adj < sig_level, lin_coeff > 0) %>% select(ensembl_gene_id)
...
#plot enriched GO parent terms  
  parentGO_plot <- 
    ego_red_sign_total %>% dplyr::arrange(desc(n)) %>% head(n=25) %>% #in case many parent terms 
    ggplot( 
 ...
 ggsave(GO_dotplot, file = paste0(res_dir, "/plot_GOenrich.pdf" ))  
  ...
  write_tsv(ego_red_sign_total, file = paste0(res_dir, "ego_red_sign_total.txt"))
...
age_trend_genes_linear_adj <- read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/age_trend_linear.txt"  )
...
#Experience: When absolute(lin_coeff) and "scoreType= "pos"" is used, we get no significant enrichment
...
totalGenes <- length(unique(geneList))

#plot enriched GO parent terms  
  parentGO_plot <- 
   gse_red_sign_total %>%  dplyr::arrange(desc(n)) %>% head(n=30) %>% #in case many parent terms 
    ggplot(
         aes(x=n/totalGenes, y=reorder(parentTerm, n, FUN=mean))
         ) + 
    geom_point(aes(colour= p.adjust, size= n)) + 
...
age_trend_genes_linear_adj_OFC <- read_tsv("~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/age_trend_linear.txt")
...
```{r GWAS overlap BD 100 kb}

BD_GWAS_100kb <- read_tsv("~/PhD/GWAS-BD/PGC3/PGC3_loci_genes_100kb.txt")
sig_level <- 0.01

analysis <- "age_trend_genes_CC/overlap_BD_GWAS"
age_trend_genes_linear_adj <- read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/age_trend_linear.txt"  )
cpm_all_long <- cpm_all_long_CC

# analysis <- "age_trend_genes_OFC/overlap_BD_GWAS"
# age_trend_genes_linear_adj <- read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/age_trend_linear.txt"  )
# cpm_all_long <- cpm_all_long_OFC
# 
...
age_trend_genes_linear_adj <- 
  age_trend_genes_linear_adj %>% 
  left_join(select(genemap, ensembl_gene_id, external_gene_name), by= "ensembl_gene_id") %>% 
  filter(external_gene_name != "") 
  

#genes with linear age trend within 100kb from GWAS loci, significant linear (nominal)
age_trend_genes_linear_BD_GWAS <-   
  age_trend_genes_linear_adj %>% 
  filter(p< sig_level) %>% 
  filter(ensembl_gene_id %in% BD_GWAS_100kb$ensgene) %>% 
  filter(ensembl_gene_id %in% cpm_all_long$ensembl_gene_id) %>% 
   filter(p< sig_level)
  
GOI_BD_GWAS <- age_trend_genes_linear_BD_GWAS %>% select(p, external_gene_name) %>% arrange(p) %>% head(n=6) %>% select(external_gene_name)

  #visualise expression when nearest gene has age trend: OFC
 expr_age_GWAS_closest <-
   cpm_all_long %>% 
   filter(gene_id %in% GOI_BD_GWAS$external_gene_name) %>% 
   ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
    geom_point()+ 
    facet_wrap(~gene_id, scales = "free_y") +
    geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE) 
 
#genes with linear age-trend (~%)
#genes within GWAS loci windows (100kb)
#GWAS-loci genes with linear age-trend (~%)

total_genes <- length(unique(age_trend_genes_linear_adj$external_gene_name))
total_trend <- length(age_trend_genes_linear_adj$external_gene_name[age_trend_genes_linear_adj$p < sig_level])
trend_GWAS <- length(unique(age_trend_genes_linear_BD_GWAS$external_gene_name))
GWAS_genes <- length(unique(BD_GWAS_100kb$symbol))

result_linear_trend_GWAS <- tibble(.rows = 4)
result_linear_trend_GWAS$cat <- c("expressed genes", "genes with linear age-trend", "genes within GWAS loci windows (100kb)", "GWAS-loci genes with linear age-trend")
result_linear_trend_GWAS$value <- c(total_genes, total_trend, GWAS_genes ,trend_GWAS)

# ggsave(expr_age_GWAS_closest, file = paste0(res_dir, "/expr_age_GWAS_closest.pdf" ), width = 120, units = "mm")  
 
 #readr::write_tsv(as_tibble(result_linear_trend_GWAS), file  = paste0(res_dir, "result_linear_trend_GWAS.txt") )
...
#analysis <- "age_trend_genes_CC/overlap_SCZ_GWAS"
#age_trend_genes_linear_adj <- read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_CC/age_trend_linear.txt"  )
#cpm_all_long <- cpm_all_long_CC

analysis <- "age_trend_genes_OFC/overlap_SCZ_GWAS"
age_trend_genes_linear_adj <- read_tsv( file ="~/proj_Maryland_WGCNA/analysis/age_trend_genes_OFC/age_trend_linear.txt"  )
cpm_all_long <- cpm_all_long_OFC
...
#genes with linear age trend within 100kb from GWAS loci, significant linear (nominal)
age_trend_genes_linear_SCZ_GWAS <-   
  age_trend_genes_linear_adj %>% 
  filter(p< sig_level) %>% 
  filter(ensembl_gene_id %in% SCZ_GWAS_100kb$`Gene stable ID` ) %>% 
  filter(ensembl_gene_id %in% cpm_all_long$ensembl_gene_id) %>% 
   filter(p< sig_level)
  
GOI_SCZ_GWAS <- age_trend_genes_linear_SCZ_GWAS %>% select(p, external_gene_name) %>% arrange(p) %>% head(n=6) %>% select(external_gene_name)

  #visualise expression when nearest gene has age trend: OFC
 expr_age_GWAS_closest <-
   cpm_all_long %>% 
   filter(gene_id %in% GOI_SCZ_GWAS$external_gene_name) %>% 
   ggplot(aes(x=AGE, y= log_cpm, colour=gene_id)) +
    geom_point()+ 
    facet_wrap(~gene_id, scales = "free_y") +
    geom_smooth(method = lm, formula = y ~ poly(x, 2), se = FALSE) 
...
total_genes <- length(unique(age_trend_genes_linear_adj$external_gene_name))
total_trend <- length(age_trend_genes_linear_adj$external_gene_name[age_trend_genes_linear_adj$p < sig_level])
trend_GWAS <- length(unique(age_trend_genes_linear_SCZ_GWAS$external_gene_name))
GWAS_genes <- length(unique(SCZ_GWAS_100kb$`HGNC symbol`))
...
#
# Hvis jeg forstår spørsmålet ditt riktig, så kan du kjøre en Fisher-test i R. Hvis vi for eksempel antar at 117 av 20,000 gener i GO-databasen er involvert i myelinering, og at du finner 100 gener med en signifikant linear trend over tid (FDR<0.05), hvorav 10 gener overlapper med de 117 myelineringsgenene, kan du gjøre følgende i R:
# 
# dat <- data.frame(
#     myelin_yes = c(10, 107),
#     myelin_no = c(90, 19793),
#     row.names = c("trend_yes", "trend_no")
# )
#fisher.test(dat)

overlap_n <- length(myelin_genes$`ENSEMBL ID`[myelin_genes$`ENSEMBL ID` %in% c(pos_linear$ensembl_gene_id, neg_linear$ensembl_gene_id)])
myelin_n <- length(myelin_genes$`ENSEMBL ID`)
total_genes <- length(unique(age_trend_genes_linear_adj$ensembl_gene_id))
GOI_n <- length(unique(c(pos_linear$ensembl_gene_id, neg_linear$ensembl_gene_id)))

#fisher test of enrichment, myelination genes
 dat <- data.frame(
     myelin_yes = c(overlap_n, myelin_n - overlap_n ),
     myelin_no = c(GOI_n -overlap_n, total_genes - myelin_n),
     row.names = c("trend_yes", "trend_no")
 )
...
```{r synaptic plasticity genes}
synaptic_plast_genes <- read_csv("~/PhD/RNAseq_relative/SCZ/functional_sets/Tuomo_synaptic_plasticity/genes_new.txt", col_names = FALSE)
synaptic_plast_genes <- synaptic_plast_genes %>% rename(external_gene_name = X1)

synaptic_plast_genes <- synaptic_plast_genes %>% 
  left_join(select(genemap, ensembl_gene_id, external_gene_name), by= "external_gene_name") %>% 
  filter(external_gene_name != "")

overlap_n <- length(synaptic_plast_genes$ensembl_gene_id[synaptic_plast_genes$ensembl_gene_id %in% c(pos_linear$ensembl_gene_id, neg_linear$ensembl_gene_id)])
synaptic_plast_n <- length(synaptic_plast_genes$ensembl_gene_id)
total_genes <- length(unique(age_trend_genes_linear_adj$ensembl_gene_id))
GOI_n <- length(unique(c(pos_linear$ensembl_gene_id, neg_linear$ensembl_gene_id)))

#fisher test of enrichment, synaptic plasticity genes
 dat <- data.frame(
     synaptic_yes = c(overlap_n, synaptic_plast_n - overlap_n ),
     synaptic_no = c(GOI_n -overlap_n, total_genes - synaptic_plast_n),
     row.names = c("trend_yes", "trend_no")
 )
fisher.test(dat, alternative = "greater")
...
XCould we find examples of genes with higher variance below 12?
